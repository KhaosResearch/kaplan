...
from kapylan.problem.msa_problem.substitution_matrix import SubstitutionMatrix, PAM250
from kapylan.problem.msa_problem.tool import StrikeEx
...
class Score:
    ...
    def compute(self, align_sequences: list) -> float:
        ...
        """
        if not all(
            len(sequence) == len(align_sequences[0]) for sequence in align_sequences
        ):
    ...
    @staticmethod
    def get_score_of_two_chars(
        substitution_matrix: SubstitutionMatrix, char_a: str, char_b: str
    ) -> int:
...
class Entropy(Score):
    ...
    def get_column_entropy(column: dict) -> float:
        ...
        for key, value in column.items():
            ...
            logger.debug(
                "Character {0}, frecuency: {1}, entropy: {2})".format(
                    key, value, value * math.log(value)
                )
            )
...
class Star(Score):
    ...
    def get_score_of_k_column(self, column: list) -> int:
        ...
        for char in column:
            score_of_column += self.get_score_of_two_chars(
                self.substitution_matrix, most_frequent_char, char
            )
...
class SumOfPairs(Score):
    ...
    def get_score_of_k_column(self, column: list) -> int:
        ...
        for char_a, char_b in self.__possible_combinations(column):
            score_of_column += self.get_score_of_two_chars(
                self.substitution_matrix, char_a, char_b
            )
...
class PercentageOfNonGaps(Score):
    def evaluate(self, align_sequences: list) -> float:
        ...
        logger.debug(
            "Total number of non-gaps: {0}".format(length_of_sequence * 2 - no_of_gaps)
        )
...
class PercentageOfTotallyConservedColumns(Score):
    def evaluate(self, align_sequences: list) -> float:
        ...
        logger.debug(
            "Total number of conserved columns: {0} out of {1}".format(
...
class Strike:
    ...
    def __compute(
        self, align_sequences: list, sequences_id: list, chains: list
    ) -> float:
        # Check if directory exists (otherwise, create it)
        os.makedirs("strike/", exist_ok=True)

        if (
            not Path(self.out_connection_path).is_file()
            and not Path(self.out_alignment_path).is_file()
        ):
            with open(self.out_connection_path, "w+") as c_file, open(
                self.out_alignment_path, "w+"
            ) as a_file:

...
class Tool:
    ...
    def run(self, parameters: dict):
        if self.__exe_exists():
            ...
            raise Exception(
                "{0} executable could been found on path {1}".format(
                    self.exe, self.exe_path
                )
            )
    ...
    def __create_command(self, parameters: dict) -> str:
        return self.exe + "".join(
            " {} {} ".format(key, val) for key, val in parameters.items()
        )
...
class StrikeEx(Tool):
    def __init__(
        self,
        exe: str = "strike",
        full_name: str = "Single structure induced evaluation",

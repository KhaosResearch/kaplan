        ...
        if char1 is self.gap_character and char2 is self.gap_character:
            ...
            try:
                distance = (
                    ...
                    else matrix[(char2, char1)]
                )
            except KeyError:
                raise Exception(
                    "The pair ({0},{1}) couldn't be found in the substitution matrix".format(
                        char1, char2
                    )
                )
...
class FileMatrix(SubstitutionMatrix):
    ...
    def __init__(
        self, path_to_file: str, gap_penalty: int = -8, gap_character: str = "-"
    ):
                    ...
                    if not line.startswith("#") and line.strip():
                        ...
                        if line.startswith(" "):
                            ...
                            for i in range(len(header) - 1):
                                if (tmp[0], header[i + 1]) not in matrix and (
                                    header[i + 1],
                                    tmp[0],
                                ) not in matrix:
                                    distance_matrix.update(
                                        {(tmp[0], header[i]): int(tmp[i + 1])}
                                    )

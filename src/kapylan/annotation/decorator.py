...
def annotated_component(uri, predict: Dict[str, str], object: Dict[str, str], **kwargs):
    ...
    def decorator(func):
        ...
        g.add((name_implementation, BIGOWL.namespace.module, rdflib.Literal("drama_kaplan.catalog.publisher.operator.Operator", datatype=XSD.string)))
        g.add((name_implementation, RDF.namespace.type, BIGOWL.namespace.Implementation))
        g.add((name_implementation, BIGOWL.namespace.implementationLanguage, rdflib.Literal("Python", datatype=XSD.string)))
        setattr(func, '__triples__', g)
        return func

    return decorator

def component(uri, predict: Dict[str, str] = None, object: Dict[str, str] = None, **kwargs):
...
def parameter(uri):
    ...
    g.add((uri, RDF.namespace.type, rdflib.URIRef("http://www.e-lico.eu/ontologies/dmo/DMOP/DMOP.owl#Parameter")))
    g.add((uri, RDFS.label, rdflib.Literal(stringcase.titlecase(name), datatype=XSD.string)))
    return g

...
from kaplan.algorithm.component.variation.variation import Variation
from kaplan.annotation.component_annotation import VariationComponent
from kaplan.annotation.decorator import merge_component
from kaplan.annotation.ontology import ontology
from kaplan.core.operator import Crossover, Mutation
...
CrossoverAndMutationVariationComponent = merge_component(VariationComponent, {
...
@CrossoverAndMutationVariationComponent(hasImplementation=TITAN.namespace.ImplementationCrossoverAndMutationVariation, label=rdflib.Literal('Crossover and Mutation Variation', datatype=XSD.string))
class CrossoverAndMutationVariation(Variation, ABC):
    def __init__(self, mutation_operator: Mutation, crossover_operator: Crossover, offspring_population_size: int):
        super(CrossoverAndMutationVariation, self).__init__(mutation_operator, crossover_operator)
        self.mutation_operator = mutation_operator
        self.crossover_operator = crossover_operator
        self.offspring_population_size = offspring_population_size

        self.mating_pool_size = int(self.offspring_population_size * self.crossover_operator.get_number_of_parents() / self.crossover_operator.get_number_of_children())

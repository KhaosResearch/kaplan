...
class RunningAlgorithmTestCase(unittest.TestCase):
    def setUp(self):
        ...
        self.variation = CrossoverAndMutationVariation(
            self.mutation, self.crossover, self.offspring_size
        )
        self.solution_creation = RandomSolutionCreation(
            self.problem, self.population_size, RandomGenerator()
        )
        self.termination = StoppingByEvaluations(max_evaluations=self.max_evaluations)
        self.replacement_policy = RankingAndDensityEstimatorReplacement(
            FastNonDominatedRanking(DominanceComparator()),
            CrowdingDistance(),
            RemovalPolicyType.ONE_SHOT,
        )
        self.selection = BinaryTournamentSelection(
            self.variation.get_mating_pool_size(), DominanceComparator()
        )
...
class IntegrationTestCases(unittest.TestCase):
    def test_should_NSGAII_Evolutionary_Algorithm_work_when_solving_problem_ZDT1_with_standard_settings(
        self,
    ):
        problem = ZDT1()
        ...
        algorithm = EvolutionaryAlgorithm(
            name="NSGA-II",
            solution_creation=RandomSolutionCreation(
                problem, population_size, RandomGenerator()
            ),
            variation=variation,
